...
public class AsyncInteractor implements IAsyncInteractor {
  ...
  /*  public void validateCredentialsMultipartAsync(final OnRequestListener listener, final int pid, final
    ...
    public void getJsonObjectResponse(final OnRequestListener listener, final int pid, String url) {
        ...
        if (pid == AppConstants.TAG_ID_CLASS_LIST) {
            Log.i(TAG, "TAG_ID_CLASS_LIST" + url);
            AppController.getInstance().getRequestQueue().getCache().invalidate(url, true);
            StringRequest stringRequest = new StringRequest(Request.Method.GET, url,
                    new Response.Listener<String>() {
                        @Override
                        public void onResponse(String response) {
                            Log.i(TAG, "TAG_ID_CLASS_LIST" + response.toString());
        ...
        if (pid == AppConstants.TAG_ID_CLASS_LIST_UPDATED) {
            Log.i(TAG, "TAG_ID_CLASS_LIST_UPDATED" + url);
            AppController.getInstance().getRequestQueue().getCache().invalidate(url, true);
            StringRequest stringRequest = new StringRequest(Request.Method.GET, url,
                    new Response.Listener<String>() {
                        @Override
                        public void onResponse(String response) {
                            Log.i(TAG, "TAG_ID_CLASS_LIST_UPDATED" + response.toString());
                            System.out.println(response);
                            try {
                                listener.onRequestCompletion(pid, response.toString());
                            } catch (Exception e) {
            ...
            // Adding request to volley request queue
            AppController.getInstance().addToRequestQueue(stringRequest);
        } else if (pid == AppConstants.TAG_ID_SUBJECT_LIST) {
            Log.i(TAG, "TAG_ID_SUBJECT_LIST" + url);
            AppController.getInstance().getRequestQueue().getCache().invalidate(url, true);
            StringRequest stringRequest = new StringRequest(Request.Method.GET, url,
                    new Response.Listener<String>() {
                        @Override
                        public void onResponse(String response) {
                            Log.i(TAG, "TAG_ID_SUBJECT_LIST" + response.toString());
                            System.out.println(response);
                            try {
                                listener.onRequestCompletion(pid, response.toString());
                            } catch (Exception e) {
            ...
            // Adding request to volley request queue
            AppController.getInstance().addToRequestQueue(stringRequest);
        } else if (pid == AppConstants.TAG_ID_DELETE_A_QUESTION_PAPER) {
            Log.i(TAG, "TAG_ID_DELETE_A_QUESTION_PAPER" + url);
            StringRequest stringRequest = new StringRequest(Request.Method.DELETE, url,
                    new Response.Listener<String>() {
                        @Override
                        public void onResponse(String response) {
                            Log.i(TAG, "TAG_ID_DELETE_A_QUESTION_PAPER" + response.toString());
                            System.out.println(response);
                            try {
                                listener.onRequestCompletion(pid, response.toString());
                            } catch (Exception e) {
    ...
    public void onPostMethodServerCall(final OnRequestListener listener, final int pid,
        ...
        if (pid == AppConstants.TAG_ID_LOGIN) {
            ...
            // Adding request to volley request queue
            AppController.getInstance().addToRequestQueue(stringRequest);
        } else if (pid == AppConstants.TAG_ID_FORGOT_PASSWORD_RESEND_OTP) {
            StringRequest stringRequest = new StringRequest(Request.Method.POST, url,
                    new Response.Listener<String>() {
                        @Override
                        public void onResponse(String response) {
                    ...
                    new Response.ErrorListener() {
                        ...
                        public void onErrorResponse(VolleyError error) {
                            ...
                            if (error.networkResponse.data != null) {
                                try {
                                    body = new String(error.networkResponse.data, "UTF-8");
                                    Log.d("FORGOT_Pswd_resend_otp", "error --" + body);
                                    listener.onRequestCompletionError(pid, body);
                                } catch (UnsupportedEncodingException e) {
                                    error.printStackTrace();
                                }
            ...
            // Adding request to volley request queue
            AppController.getInstance().addToRequestQueue(stringRequest);
        } else if (pid == AppConstants.TAG_ID_SET_PASSWORD) {
            StringRequest stringRequest = new StringRequest(Request.Method.POST, url,
                    new Response.Listener<String>() {
                        @Override
                        public void onResponse(String response) {
